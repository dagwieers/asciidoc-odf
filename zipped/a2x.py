...
def shell(cmd, raise_error=True, want_return=True):
	'''
	Execute command cmd in shell and return resulting subprocess.Popen object.
	If want_return is true, always set stdout to PIPE irrespective of verbose
...
class A2X(AttrDict):
	...
	def load_conf(self):
		...
		if self.backend is not None:
			ret, err = shell(self.asciidoc + " --backend list", want_return=True)
			backends = [(i, os.path.split(i)[1]) for i in ret.splitlines()]
			backend_dir = [i[0] for i in backends if i[1] == self.backend]
			if len(backend_dir) != 1:
				die('backend mismatch, %i found for backend: %s' % (len(backend_dir), self.backend))
			verbose('found backend directory: %s' % backend_dir[0] )
			conf_files.append(os.path.join(backend_dir[0], CONF_FILE))
...
#		self.xsltproc_opts += ' --nonet'
	...
	def to_backend(self):
		eval("to_"+self.backend+"(self)") # evaluate global function in class context
...
if __name__ == '__main__':
	...
	parser.add_option('--backend_opts',
