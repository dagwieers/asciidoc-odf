...
However if you prefer annotations inside the ODF file, you can use the
+-a annotations+ option on the command line or add the +annotations+
attribute yo your AsciiDoc file:

    :annotations:

[NOTE]
Beware that annotations are only added to the ODF output and will not be
printed, or might be removed depending on the conversion (e.g. to PDF).
+
If you insist on having comments forced in the output (e.g. for
collaborating on a draft document) then use the +showcomments+ attribute.
